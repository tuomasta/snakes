...
type IClientApi = abstract member GameState: Shared.DTO.GameDto -> Tasks.Task
...
type GameHub() =
    ...
    static let ConnectionData = ConcurrentDictionary<string, ConnectionData>()
        ...
        task {
            ...
            let doAccept _ =
                async {
                    ...
                    let connectionData = { GameId = game.name; PlayerId = player.id }
                    let update _ __ = connectionData
                    ConnectionData.AddOrUpdate(connectionId, connectionData, update) |> ignore
    ...
    override this.OnDisconnectedAsync e =
            ...
            let deletePlayer data = getRedisDb |> deletePlayer data.GameId data.PlayerId
    ...
    member this.Turn(direction: TurnDirection) =
        task {
            let turnPlayer (game:Game, data) = 
                let player = game.players |> List.find (fun p -> p.id = data.PlayerId)
                player.turn direction
                getRedisDb |> setPlayerDirection game.name player.id player.direction
