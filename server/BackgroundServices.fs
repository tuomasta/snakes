...
type GameUpdateProxyService(hubContext: IHubContext<GameHub, IClientApi>) =
    ...
    let handleGameUpdate (dto: Shared.DTO.GameDto) =
        hubContext.Clients.Group(dto.name).GameState(dto)
        |> Async.AwaitTask
        |> Async.RunSynchronously

        ()

    let mutable unsubscriber: (unit -> Async<unit>) =
        fun () -> async.Return()
    ...
    override this.StartAsync _ =
        task {
            ...
            let! sub =
                getRedisSubscriber
                |> subscribeGameState handleGameUpdate

            unsubscriber <- sub
        }

    override this.StopAsync _ = task { do! unsubscriber () }
