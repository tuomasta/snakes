...
module Game =
    ...
    module DAO =
        ...
        let fromDAO (dao: GameDAO) : Game =
            ...
            let mapPlayer _ (pDAO: PlayerDAO) : Player =
                ...
                let player: Player =
                    Player(pDAO.id, pDAO.name, head, direction)
    ...
    let tryGetGame name getDb =
        getDb
        |> tryGetJson<DAO.GameDAO> name
        |> Option.bindAsync2 DAO.fromDAO

    let saveGame (game: Game) (getDb: GetDb) =
        async {
    ...
    let getOrCreateGame name getDb =
        // todo make transactional
        tryGetGame name getDb
        |> Option.defaultWithAsync (fun () -> saveGame (name |> Game) getDb)
    ...
    let getGameNames (getDb: GetDb) : Async<string seq> =
        ...
        |> Async.mapAsync (fun hashes ->
            (hashes
             |> Seq.map (fun hash -> hash.Name.ToString())))
    ...
    let private gameStateSubscriptionKey =
        "snake:gamestate"
